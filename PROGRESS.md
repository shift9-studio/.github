...
**Position:** 131 commits ahead, 0 behind. Working tree clean.
**Scope:** 72 files, +7309 / −457. 62 of them under `shift9/apps/shift9-dev`.
...
## Known, and NOT introduced by this branch
...
- **`href="/#work"` is a dead anchor.** Used twice in `app/work/[slug]/page.tsx`
  (lines 68 and 228). No `id="work"` exists on `/` or `/studio` — and none exists on
  `main` either. This branch does change *where it dumps you*: `/` is now the
  entrance gate, so "← ALL WORK" sends a deep-linked visitor to the front door.
...
- **No favicon.** `/favicon.ico`, `/icon.svg` and `/apple-icon.png` all 404 on both
  apps. This is the single 404 in the console on `/`.

## Known, and introduced by this branch

- **The desktop clock is hardcoded** — `9:41 AM / 7/14/2026`
  (`EnterTheStudio.tsx:891`). 9:41 is the deliberate convention; the date is not,
  and it reads as stale the further past it we get.
