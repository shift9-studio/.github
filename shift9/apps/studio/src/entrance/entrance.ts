/* SHIFT-9 entrance — video autoplays → first-person desk POV → amigurumi hands open the
   laptop and grab the wireless mouse → camera zooms into the screen. 1:1 port of
   reference/shift9-entrance.js on WebGPURenderer, fidelity raised: soft shadows,
   full-DPR, stitch-row knit bump texture on the hands.
   Audio pipeline is swappable: entrance music is layered OVER the muted video via the
   audio engine (the video file itself stays untouched). */
...
export class Entrance {
  ...
  constructor(host: HTMLElement, audio: AudioEngine) {
    ...
    this.root.className = 's9-entrance';
    host.appendChild(this.root);

    const v = this.video = document.createElement('video');
    v.src = '/assets/entrance-video.mp4';
    v.playsInline = true; v.autoplay = true;
    this.root.appendChild(v);
    // Music replacement (Known Open Item): mute the video's own track and lay the
    // score under it; if unmuted autoplay is blocked, we were muted anyway.
    v.muted = true;
    v.play().then(() => this.audio.startEntranceScore(v)).catch(() => this.toDesk());
    v.addEventListener('ended', () => this.toDesk());
    v.addEventListener('error', () => this.toDesk());
  }

  private toDesk() {
    if (this.desk || this.disposed) return; this.desk = true;
    this.audio.endEntranceScore();
    const v = this.video;
    if (v) {
      v.style.transition = 'opacity 0.9s'; v.style.opacity = '0';
      setTimeout(() => { try { v.remove(); } catch { /* already gone */ } }, 950);
    }
    void this.startDesk();
  }

  skipToDesk() { this.toDesk(); }

  private async startDesk() {
    const forceWebGL = new URLSearchParams(location.search).has('webgl');
    const renderer = this.renderer = new THREE.WebGPURenderer({ antialias: true, forceWebGL });
    await renderer.init();
    ...
    renderer.domElement.style.transition = 'opacity 1.2s';
    ...
    resize(); this.ro = new ResizeObserver(resize); this.ro.observe(this.root);
    // ── lighting
    ...
    warm.target.position.set(0, 0, 0); warm.castShadow = true; warm.shadow.mapSize.set(2048, 2048); warm.shadow.bias = -0.0004;
    ...
    for (let i = 0; i < 60; i++) {
      wcx.strokeStyle = `rgba(${20 + Math.random() * 30 | 0},${14 + Math.random() * 20 | 0},${8 + Math.random() * 14 | 0},0.5)`;
      wcx.lineWidth = 1 + Math.random() * 2; wcx.beginPath();
      const y = Math.random() * 256; wcx.moveTo(0, y);
      wcx.bezierCurveTo(80, y + (Math.random() - 0.5) * 14, 170, y + (Math.random() - 0.5) * 14, 256, y + (Math.random() - 0.5) * 8);
      wcx.stroke();
    }
    ...
    const drawScreen = (on: boolean, tt: number) => {
      ...
      if (on) { // b&w galaxy boot glow
        ...
        scx.fillStyle = '#fff';
        for (let i = 0; i < 40; i++) {
          const hx = Math.abs(Math.sin(i * 127.1) * 43758.5) % 1, hy = Math.abs(Math.sin(i * 311.7) * 12543.3) % 1;
          scx.globalAlpha = 0.2 + (i % 5) * 0.15; scx.fillRect(hx * 512, hy * 320, 2, 2);
        }
    ...
    // ── desk gear: dial, Edge, coaster + mug, headphones, monitors
    ...
    const drawEdge = (mode: 'calendar' | 'news', news?: string[]) => {
      ...
      if (mode === 'news' && news) {                       // news path: pass an array of headline strings
        ...
        'SMTWTFS'.split('').forEach((ch, i) => {
          ...
          ecx.fillStyle = today ? '#04141a' : '#8a8e98'; ecx.font = '600 12px sans-serif'; ecx.fillText(ch, x, 356);
        });
    ...
    const mkSpeaker = (x: number, z: number, ry: number) => {
    ...
    // stitch rows read at production DPR via a denser knit bump texture
    const kc = document.createElement('canvas'); kc.width = kc.height = 128;
    const kcx = kc.getContext('2d')!;
    kcx.fillStyle = '#888'; kcx.fillRect(0, 0, 128, 128);
    for (let y = 0; y < 16; y++) for (let x = 0; x < 16; x++) {
      kcx.fillStyle = (x + y) % 2 ? '#666' : '#aaa';
      kcx.beginPath(); kcx.ellipse(x * 8 + 4 + (y % 2) * 3, y * 8 + 4, 3.4, 2.4, 0.6, 0, 7); kcx.fill();
    }
    ...
    const mkHand = (mirror: boolean, accessory: 'bracelets' | 'watch') => {
      ...
      for (let i = 0; i < 4; i++) {
        const f = new THREE.Mesh(new THREE.SphereGeometry(0.0115, 12, 10), yarn);  // SLIM sphere fingers
        f.scale.set(0.9, 0.8, 2.2); f.position.set(-0.0255 + i * 0.017, 0.003, -0.046); h.add(f);
      }
      ...
      const wrist = (d: number) => ({ x: 0, y: -0.012 - 0.247 * d, z: 0.055 + 0.969 * d });   // point d along the arm axis
    ...
    hint.className = 's9-hint';
    this.root.appendChild(hint);
    setTimeout(() => { hint.style.opacity = '1'; }, 1600);
    let phase: 'idle' | 'open' | 'mouse' | 'hold' | 'zoom' = 'idle';
    let pt = 0, mx = 0, my = 0, tt = 0, fired = false, lidFoley = false, boot = false;
    ...
    const edgeAt = (ang: number) => lidEdge.set(0, 0.022 + 0.42 * Math.sin(ang), -0.23 + 0.42 * Math.cos(ang));
    const mouseGrip = new THREE.Vector3(0.46, 0.045, 0.275);
    let last = performance.now();
    let hrGrip = new THREE.Vector3(), hrFrom = new THREE.Vector3();
    let em = -1;
    const loop = (now: number) => {
      this.raf = requestAnimationFrame(loop);
      const dt = Math.min((now - last) / 1000, 0.05); last = now; tt += dt;
      const min = Date.now() / 60000 | 0;
      if (min !== em) { em = min; drawEdge('calendar'); }   // keep the Xeneon Edge clock live
      if (phase === 'idle') {
        ...
        (ring.material as THREE.MeshBasicMaterial).color.setHSL(0.52, 0.9, 0.5 + 0.12 * Math.sin(tt * 2));
      } else if (phase === 'open') {                       // hands reach in, right hand lifts the lid
        pt += dt; const k = Math.min(pt / 1.7, 1);
        if (!lidFoley) { lidFoley = true; this.audio.lidFoley(); }
        ...
        if (lidK > 0.3) {
          if (!boot) { boot = true; this.audio.bootChime(); }
          drawScreen(true, tt); sLight.intensity = 4 * lidK;
        }
        camera.lookAt(0, 0.1 + 0.1 * lidK, -0.05);
        if (k >= 1) { phase = 'mouse'; pt = 0; hrFrom = handR.position.clone(); }
      } else if (phase === 'mouse') {                      // right hand glides onto the wireless mouse
  ...
  /* Built-in seam kept for production: switch the Xeneon Edge canvas to a news feed. */
