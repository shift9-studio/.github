...
assert.match(room, /drawer_cabinet/, "Room explore must keep the printer-bay cabinet fallback");
assert.match(room, /wood_table_diff\.jpg/, "Room explore must use Poly Haven wood PBR, not a gray box");
assert.match(room, /enclosed_printer\.glb/, "Room explore must keep a reconstructed enclosed printer fallback");
assert.match(room, /flat_monitor\.glb/, "Room explore must keep a reconstructed monitor fallback");
