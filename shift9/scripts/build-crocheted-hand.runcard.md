...
> **Revision 3.** Kariim's direction, twice: it is a DOLL'S hand *with real
> fingers* and must never read as a real hand in a knitted glove — then "refine
> the hand more". Revision 1 was a fused mitten; revision 2 gave it four
> worked-in-the-round digits plus a thumb; revision 3 is about everything
> *around* the fingers, because in the room the fingers were correct and
> invisible.
...
- **The forearm is 340mm**, which is long for a forearm on a 96mm hand. It is
  deliberate: the tube is open at the elbow end, every millimetre past the frame
  edge is invisible, and the alternative is a visible flat cap. Verified
  off-frame at the widest window the room admits (2:1) with 36mm to spare.
- **The sleeve stays dark in the room** even with the monitor lighting it. That
  is not a defect — it is a near-black knit (`--s9-yarn-cuff`) in a dim room,
  backlit by the only bright thing in it. The knit reads along the top and upper
  face and the silhouette is clean; the near face is genuinely in shadow.
- **The pose is one `rotation` in `props/hand.tsx`**, and the comment there
  records both failure modes it is bracketed by. Reversible in one number.
